...
/**
 * <p>
 * Multiple-Instance Nearest Neighbour with Distribution learner. Attention:
 * Cannot handle numeric attributes! WEKA: "It uses gradient descent to find
 * the weight for each dimension of each exeamplar from the starting point of
 * 1.0. In order to avoid overfitting, it uses mean-square function (i.e. the
 * Euclidean distance) to search for the weights. It then uses the weights to
 * cleanse the training data. After that it searches for the weights again
 * from the starting points of the weights searched before. Finally it uses
 * the most updated weights to cleanse the test exemplar and then finds the
 * nearest neighbour of the test exemplar using partly-weighted Kullback
 * distance. But the variances in the Kullback distance are the ones before
 * cleansing." Xin Xu (2001). A nearest distribution approach to multiple-
 * instance learning. Hamilton, NZ.
 * </p>
 *
 * @version 1.01
 * @since 2018
